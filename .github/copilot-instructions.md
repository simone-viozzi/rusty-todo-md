...
- Coverage is **informational only** — codecov is configured (see `.codecov.yml`) to never fail CI or post status comments. The primary correctness signal is the test suite, in particular the `insta` snapshot tests under `tests/snapshot_tests.rs`.
