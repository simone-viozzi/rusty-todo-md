...
def main() -> int:
    ...
    lines.append("")
    lines.append("**Outcome.** After the snapshot corpus expanded from 5 → 23 fixtures (#207),")
    lines.append("the subagent fan-out returned **DELETE for 11 of 32** `tests/*.rs` candidates")
    lines.append("with overlap ≥ 0.70 — the integration tests those snapshots now subsume on")
    lines.append("both stdout and stderr. See `triage-verdicts.md` for the full per-candidate")
    lines.append("breakdown; the deleted tests are removed in this PR.")
    lines.append("")
